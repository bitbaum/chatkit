...
export function parseBlocks(text) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (/^#{1,6}\s/.test(line)) {
            ...
            if (prev && prev.kind === "p" && (lines[i - 1] ?? "").trim())
                prev.text += `\n${line}`;
            else
...
const INLINE_RE = /(`[^`]+`|\*\*[^*]+\*\*|\*[^*\s][^*]*\*|\[[^\]]+\]\([^)\s]+\)|\[[FD]\d+(?:,\s*[FD]\d+)*\]|https?:\/\/[^\s<>()]+[^\s<>().,;:!?'"])/g;
        ...
        if (/^\[[FD]\d+(?:,\s*[FD]\d+)*\]$/.test(part)) {
            ...
            const href = link
                ...
                : /^https?:\/\//.test(part)
                    ...
                    : null;
            if (link && href)
                out.push({ kind: "link", text: link[1] ?? href, href });
            else if (!link && href)
                out.push({ kind: "link", text: part, href });
            else
                out.push({ kind: "text", text: link ? (link[1] ?? part) : part });
        }
    }
    return out;
}
