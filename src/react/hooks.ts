 ...
 */
export function useStickToBottom(changes: DependencyList, grew: number) {
  ...
  const onScroll = useCallback(() => {
    ...
    if (el) setFollowing(isFollowing(el));
  }, []);
  ...
  useEffect(() => {
    if (grew > previous.current) setFollowing(true);
 ...
 */
export function useViewportHeight(target?: RefObject<HTMLElement | null>) {
  useEffect(() => {
...
export function useAutoGrow(ref: RefObject<HTMLTextAreaElement | null>, value: string, max = 240) {
  useLayoutEffect(() => {
